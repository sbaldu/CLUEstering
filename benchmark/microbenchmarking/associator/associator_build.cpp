...
static void BM_BuildBinaryAssociatorAlpaka(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::ranges::transform(std::views::iota(0, elements), h_associations.data(), [](auto x) -> int32_t {
      return x % 2 == 0;
    });
	auto d_associations = clue::make_device_buffer<int[]>(queue, elements);
	alpaka::memcpy(queue, d_associations, h_associations);
    volatile auto associator = clue::internal::make_associator(queue, std::span{d_associations.data(), elements}, elements);
  }
}
