...
import CLUE_Domains as clue_domains
...
class clusterer:
    ...
    def change_domains(self, **kwargs: tuple) -> None:
        ...
        # Construct the domains of all the coordinates
        empty_domain = clue_domains.domain_t()
        ...
        for coord, domain in kwargs.items():
            self.clust_data.domain_ranges[int(coord[1])] = \
                clue_domains.domain_t(self.scaler.transform([[domain[0]]])[0][0],
    ...
    def run_clue(self, backend: str = "cpu serial", verbose: bool = False) -> None:
        ...
        if backend == "cpu serial":
            cluster_id_is_seed = cpu_serial.mainRun(self.dc_, self.rhoc, self.outlier, self.ppbin,
                                                    ...
                                                    self.kernel, self.clust_data.domain_ranges, self.clust_data.n_dim)
