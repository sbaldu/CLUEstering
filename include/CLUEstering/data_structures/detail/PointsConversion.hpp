...
namespace clue {
  ...
  inline void copyToDevice(TQueue& queue,
    ...
    meta::apply<Ndim>([&]<std::size_t Dim>() -> void {
      if (h_points.view().has_coordinate_uncertainty(Dim)) {
        using dev_value_t = std::remove_cv_t<TDeviceInput>;
        using PType = std::remove_cvref_t<decltype(d_points)>;
        auto& cbufs = internal::points_interface<PType>::coord_uncertainty_buffers(d_points);
        ...
        alpaka::memcpy(
            ...
            make_host_view(h_points.view().m_coordinate_uncertainties[Dim], h_points.size()));
        d_points.view().m_coordinate_uncertainties[Dim] = cbufs[Dim]->data();
      }
    });
    alpaka::wait(queue);
  }
