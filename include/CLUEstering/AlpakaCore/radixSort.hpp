...
#include "alpakaWorkDim.hpp"
#include "alpakaConfig.hpp"

namespace {
  template <typename T>
  concept SingleThreadedAccelerator =
      requires(T&& t) { clue::requires_single_thread_per_block_v<TAcc>; }

  template <typename T>
  concept MultiThreadedAccelerator =
      requires(T&& t) { !clue::requires_single_thread_per_block_v<TAcc>; }

  template <typename T>
  concept Unsigned = std::unsigned_v<T>;
}  // namespace
...
namespace clue {
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void radixSortImpl(const TAcc& acc,
    ...
    if constexpr (!requires_single_thread_per_block_v<TAcc>) {
      ...
      const uint32_t threadIdxLocal(
          alpaka::getIdx<alpaka::Block, alpaka::Threads>(acc)[0u]);
      [[maybe_unused]] const uint32_t blockDimension(
      ...
      constexpr int binBits = 8, dataBits = 8 * sizeof(T),
                    totalSortingPassses = dataBits / binBits;
      ...
      while (alpaka::syncBlockThreadsPredicate<alpaka::BlockAnd>(
          acc, (currentSortingPass < totalSortingPassses))) {
        ...
        if (!threadIdxLocal &&
            1 == alpaka::getIdx<alpaka::Grid, alpaka::Blocks>(acc)[0]) {
          size_t total = 0;
          ...
          for (auto idx : independent_group_elements(acc, binsNumber)) {
            ...
            if (i >= 0) {
              ...
              if (cu[bin] == i) {
                ...
                for (int peerThreadIdx = idx; peerThreadIdx < binsNumber;
                     peerThreadIdx++) {
      ...
      if (j !=
          ind)  // odd number of sorting passes, we need to move the result to the right array (ind[])
        for (auto idx : independent_group_elements(acc, size)) {
          ind[idx] = ind2[idx];
        };
  ...
  template <MultiThreadedAccelerator TAcc,
            Unsigned T,
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void radixSort(
    ...
    using I = int;
    radixSortImpl<TAcc, I, NS>(
        acc, (I const*)(a), ind, ind2, size, reorderFloat<TAcc, I>);
  }

  template <SingleThreadedAccelerator TAcc,
            typename T,
            int NS = sizeof(T)>  // number of significant bytes to use in sorting
  ALPAKA_FN_INLINE void radixSort(
      const TAcc& acc, T const* a, uint16_t* ind, uint16_t* ind2, uint32_t size) {
    static_assert(requires_single_thread_per_block_v<TAcc>,
                  "CPU sort (not a radixSort) called wtth wrong accelerator");
    // Initialize the index array
    std::iota(ind, ind + size, 0);
    std::stable_sort(
        ind, ind + size, [a](uint16_t i0, uint16_t i1) { return a[i0] < a[i1]; });
  }
...
namespace alpaka::trait {
  ...
  struct BlockSharedMemDynSizeBytes<clue::radixSortMultiWrapper<T, NS>, TAcc> {
